...
// clear decFragment
deckFragment.querySelectorAll('.card-container').forEach(c => deckFragment.removeChild(c));
...
function run() {
  stopwatch.innerText = (min < 10 ? '0' + min : min) + 'm :' +  (sec < 10 ? '0' + sec : sec) + 's :' + (msec < 10 ? '0' + msec : msec) + 'ms';
  msec++;
  if(msec === 100) {
    msec = 0;
    sec++
